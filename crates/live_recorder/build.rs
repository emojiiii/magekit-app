...
fn main() {
    for name in ["MAGEKIT_UV_BUNDLE", "MAGEKIT_SKIP_UV_BUNDLE", "MAGEKIT_BUILD_PYTHON"] {
    ...
    if let Some(source) = env::var_os("MAGEKIT_UV_BUNDLE") {
        let source = PathBuf::from(source).canonicalize().expect("MAGEKIT_UV_BUNDLE must exist");
        println!("cargo:rerun-if-changed={}", source.display());
        assert!(fs::metadata(&source).unwrap().len() > 0, "uv bundle must not be empty");
        ...
        let candidates = env::var("MAGEKIT_BUILD_PYTHON")
            .map(|p| vec![p]).unwrap_or_else(|_| {
                if cfg!(windows) { vec!["python".into(), "python3".into()] }
                else { vec!["python3".into(), "python".into()] }
            });
        let mut success = false;
        for python in candidates {
            match Command::new(python).arg(&script).arg(env::var("TARGET").unwrap())
                .arg(&destination).status() {
                Ok(status) => { success = status.success(); break; }
                Err(e) if e.kind() == std::io::ErrorKind::NotFound => continue,
                Err(e) => panic!("cannot start uv bundler: {e}"),
            }
        }
        assert!(success, "uv bundling failed. See docs/streamlink.md; supply MAGEKIT_UV_BUNDLE for offline builds.");
    } else {
        fs::write(destination, []).expect("write empty development bundle");
        println!("cargo:warning=uv is not embedded: use MAGEKIT_UV_PATH or uv on PATH for development");
    }
}
