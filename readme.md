...
### Q: Windows 便携版直播录制失败，在哪里查看日志？

A: 新版本会将录制诊断写入 `%APPDATA%\MageKit\logs\magekit.log`。便携版同样使用此目录。日志只记录房间播号、失败阶段、重试次数和 HTTP 状态等诊断字段，不保存 Cookie 或授权流地址。v0.2.0 及更早版本没有持久录制日志，升级后新发生的录制问题才会写入文件。
