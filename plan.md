...
**目标**: 支持输入作者/频道 URL，解析并批量下载所有视频

#### 7.1 ChannelPage 页面实现

当前状态: ⏳ 待开发

- [ ] **创建 ChannelPage 组件**

  - 新建 `pages/channel/` 目录结构
  - 频道 URL 输入框
  - 解析按钮和状态显示
  - 视频列表展示区域
...
- [ ] **支持多平台频道 URL**
  ...
  - Bilibili UP 主: `space.bilibili.com/uid`
  - 其他平台的用户/频道页面
...
- [ ] **调用 yt-dlp 解析频道**
  ...
  - 使用 `--flat-playlist` 参数快速获取视频列表（不下载）
  - 解析返回的 JSON 数组获取所有视频信息
  - 支持分页加载（大型频道可能有数千视频）
...
- [ ] **yt-dlp 命令参数**
  ...
  ```bash
  # 获取频道所有视频信息（仅元数据，不下载）
  yt-dlp --flat-playlist -j "https://www.youtube.com/@username"

  # 输出格式：每行一个 JSON 对象
  # {"id": "xxxxx", "title": "视频标题", "duration": 123, ...}
  ```
...
- [ ] **视频列表数据结构**
  ...
  pub struct ChannelInfo {
      ...
      pub videos: Vec<ChannelVideoEntry>,
  }
...
- [ ] **输出模板**
  ...
  # 带序号（保持播放列表顺序）
  yt-dlp -o "%(uploader)s/%(playlist_index)s - %(title)s.%(ext)s" URL
  ```
...
- 使用 `--flat-playlist` 实现快速解析（`extract_flat=True`）
- 每行 JSON 独立解析，支持大型频道
- 复用现有 `DownloadTask` 和 `TaskQueue` 系统
- 频道视频列表使用虚拟滚动（如果视频数量很多）

**涉及文件**:

- `gui_app/src/ui/pages/channel/mod.rs` - 模块导出
- `gui_app/src/ui/pages/channel/page.rs` - ChannelPage 组件
- `gui_app/src/ui/pages/channel/widgets/` - 子组件
- `gui_app/src/ui/pages/mod.rs` - 添加 channel 模块
- `gui_app/src/ui/main_window.rs` - 路由配置
- `gui_app/src/ui/layout.rs` - 侧边栏导航添加入口
- `tool_manager/src/downloader.rs` - 添加 `get_channel_videos()` 方法
- `shared/src/types.rs` - 添加 `ChannelInfo`, `ChannelVideoEntry` 类型
...
1. 创建 `pages/channel/` 目录结构和基础组件
2. 实现频道 URL 解析功能（调用 yt-dlp --flat-playlist）
3. 实现视频列表展示和选择 UI
4. 实现批量下载功能（复用现有任务队列）
5. 添加路由和侧边栏导航入口
