...
fn create_cookie_file(
    ...
    let matching_cookie = cookies
        .iter()
        .find(|c| c.enabled && c.platform.to_lowercase() == platform_lower)?;
...
impl VideoDownloader {
    ...
    async fn get_bilibili_space_videos(
        &self,
        uid: &str,
        cookies: Option<&[PlatformCookie]>,
        ...
        let uploader_name = user_json["data"]["name"]
            .as_str()
            .unwrap_or("未知UP主")
            .to_string();
        ...
        loop {
            ...
            let videos_response = request.send().await.map_err(|e| {
                DownloadError::internal(format!("Failed to fetch videos page {}: {}", page, e))
            })?;

            let videos_json: serde_json::Value = videos_response.json().await.map_err(|e| {
                DownloadError::internal(format!("Failed to parse videos page {}: {}", page, e))
            })?;
    ...
    pub async fn get_channel_videos(
        ...
        // 打印前 1000 字符用于调试
        let preview = if json_str.len() > 1000 {
            &json_str[..1000]
        } else {
            &json_str
        };
                    ...
                    if let Ok(entry_data) = serde_json::from_str::<ChannelEntryData>(line) {
                        let entry =
                            ChannelVideoEntry {
                                ...
                                url: entry_data.url.or(entry_data.webpage_url).unwrap_or_else(
