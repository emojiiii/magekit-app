...
use crate::{
    error::{RecorderError, RecorderResult}, legacy_recorder,
    platforms::PlatformFactory, streamlink_runtime::{self, RuntimeInfo},
...
use tokio::{io::{AsyncBufReadExt, AsyncWriteExt, BufReader}, sync::{oneshot, watch}};
...
impl Default for LiveRecorder { fn default() -> Self { Self::new() } }
...
impl LiveRecorder {
    ...
    pub fn with_backend(backend: RecordingBackend) -> Self {
        Self { native: legacy_recorder::LiveRecorder::new(), backend }
    }
    ...
    pub fn with_factory(factory: PlatformFactory) -> Self {
        Self { native: legacy_recorder::LiveRecorder::with_factory(factory), backend: RecordingBackend::Native }
    }

    pub async fn start_recording(&self, url: &str, config: RecordConfig) -> RecorderResult<RecordingHandle> {
        self.start_recording_with_cookies(url, config, &[]).await
    }

    pub async fn start_recording_with_cookies(
        &self, url: &str, config: RecordConfig, cookies: &[PlatformCookie],
    ) -> RecorderResult<RecordingHandle> {
        if self.backend == RecordingBackend::Native {
            return self.native.start_recording_with_cookies(url, config, cookies).await.map(RecordingHandle::native);
        }
        ...
        let info = match probe(&runtime, url, &config, cookies).await {
            Ok(info) => info,
            Err(RecorderError::UnsupportedPlatform(_)) if self.backend == RecordingBackend::Auto => {
                return self.native.start_recording_with_cookies(url, config, cookies).await.map(RecordingHandle::native);
            }
        ...
        if config.segment_duration.is_some() || config.include_danmaku {
            return Err(RecorderError::ConfigError(
                "Streamlink 暂不支持定时分文件或弹幕；请关闭这些选项，或显式使用 native 后端".into()
            ));
        }
        if !["ts", "mp4", "mkv", "flv"].contains(&config.format.as_str()) {
            return Err(RecorderError::ConfigError("Streamlink 支持 ts/mp4/mkv/flv 输出".into()));
        }
        ...
        let reserved = tokio::fs::OpenOptions::new().write(true).create_new(true).open(&path).await?;
        drop(reserved);
        let request = json!({"mode":"record", "url":url, "config":config, "cookies":scoped_cookies(url, cookies),
                             "output":path, "ffmpeg":ffmpeg});
        let start = chrono::Utc::now();
        let initial = RecordProgress { status: RecordStatus::Connecting, start_time: Some(start),
                                       duration: 0, size: 0, speed: 0, error: None };
        ...
        Ok(RecordingHandle { running: Some(Running::Streamlink { stop: Some(stop), task, progress: rx }),
                             output_path: path, status: RecordStatus::Connecting, completion_error: None })
    }

    pub async fn check_room_status(&self, url: &str) -> RecorderResult<LiveRoomInfo> {
        self.check_room_status_with_cookies(url, &[]).await
    }
    pub async fn check_room_status_with_cookies(&self, url: &str, cookies: &[PlatformCookie]) -> RecorderResult<LiveRoomInfo> {
        if self.backend == RecordingBackend::Native {
            return self.native.check_room_status_with_cookies(url, cookies).await;
        }
        let normalized = room_url(url)?;
        let url = normalized.as_str();
        let runtime = streamlink_runtime::ensure().await?;
        match probe(&runtime, url, &RecordConfig::default(), cookies).await {
            ...
            Err(RecorderError::UnsupportedPlatform(_)) if self.backend == RecordingBackend::Auto =>
                self.native.check_room_status_with_cookies(url, cookies).await,
            Err(error) => Err(error),
        }
    }
    ...
    pub async fn get_stream_info_with_cookies(&self, url: &str, cookies: &[PlatformCookie]) -> RecorderResult<StreamInfo> {
        if self.backend == RecordingBackend::Native {
...
fn room_url(value: &str) -> RecorderResult<String> {
    ...
    if !matches!(parsed.scheme(), "http" | "https") || parsed.host_str().is_none()
        ...
        return Err(RecorderError::InvalidUrlFormat("Only HTTP(S) room URLs without embedded credentials are accepted".into()));
    }
    Ok(parsed.to_string())
}
...
fn scoped_cookies<'a>(url: &str, cookies: &'a [PlatformCookie]) -> Vec<&'a PlatformCookie> {
    ...
    cookies.iter().filter(|cookie| {
        ...
        let domain = match key.as_str() {
            "douyin" => "douyin.com".to_owned(), "bilibili" => "bilibili.com".to_owned(),
            "huya" => "huya.com".to_owned(), "douyu" => "douyu.com".to_owned(),
            "kuaishou" => "kuaishou.com".to_owned(), "twitch" => "twitch.tv".to_owned(),
            "soop" | "sooplive" => "sooplive.co.kr".to_owned(),
            "soop_global" => "sooplive.com".to_owned(), "afreeca" => "afreecatv.com".to_owned(),
            _ => {
                let url = if key.contains("://") { key } else { format!("https://{key}") };
                let Ok(parsed) = url::Url::parse(&url) else { return false };
                if !matches!(parsed.path(), "" | "/") { return false; }
...
async fn probe(runtime: &RuntimeInfo, url: &str, config: &RecordConfig, cookies: &[PlatformCookie]) -> RecorderResult<StreamInfo> {
    ...
    let message = event["message"].as_str().unwrap_or("Streamlink probe failed").to_owned();
    match event["kind"].as_str() {
        ...
        _ if output.status.code() == Some(12) => Err(RecorderError::AuthenticationRequired(message)),
        _ => Err(RecorderError::RecordingError(message)),
    }
}

fn safe_component(value: &str) -> String {
    let value: String = value.chars().take(120)
        .map(|c| if c.is_control() || "<>:\"/\\|?*".contains(c) { '_' } else { c }).collect();
    let value = value.trim().trim_matches('.');
    let base = value.split('.').next().unwrap_or("").to_ascii_uppercase();
    let reserved = ["CON", "PRN", "AUX", "NUL", "COM1", "COM2", "COM3", "COM4", "COM5", "COM6", "COM7", "COM8", "COM9",
                    "LPT1", "LPT2", "LPT3", "LPT4", "LPT5", "LPT6", "LPT7", "LPT8", "LPT9"];
    if value.is_empty() { "unknown".into() }
    else if reserved.contains(&base.as_str()) { format!("_{value}") }
    else { value.to_owned() }
}

fn output_path(room: &LiveRoomInfo, config: &RecordConfig) -> RecorderResult<PathBuf> {
    let platform = room.extra.get("streamlink_plugin").and_then(Value::as_str).unwrap_or("streamlink");
    let path = config.output_path_template
        ...
        .replace("{timestamp}", &chrono::Utc::now().format("%Y%m%d_%H%M%S_%3f").to_string());
    let path = PathBuf::from(path);
    if path.extension().and_then(|v| v.to_str()) != Some(config.format.as_str()) {
        return Err(RecorderError::ConfigError("输出文件扩展名必须与录制格式一致".into()));
    }
...
async fn cleanup_group(pid: Option<u32>) {
    ...
    #[cfg(unix)] {
        ...
        let _ = cmd.args(["-KILL", "--", &format!("-{pid}")])
            .stdout(Stdio::null()).stderr(Stdio::null()).status().await;
    }
    #[cfg(windows)] {
        let mut cmd = magekit_shared::create_tokio_command("taskkill");
        let _ = cmd.args(["/PID", &pid.to_string(), "/T", "/F"])
            .stdout(Stdio::null()).stderr(Stdio::null()).status().await;
    }
}

async fn supervise(
    runtime: RuntimeInfo, request: Value, mut stop: oneshot::Receiver<()>,
    progress: &watch::Sender<RecordProgress>, latest: &mut RecordProgress,
) -> RecorderResult<()> {
    let mut child = streamlink_runtime::worker(&runtime).spawn()?;
    let pid = child.id();
    let mut guard = ProcessGuard(pid);
...
enum Running {
    ...
    Streamlink { stop: Option<oneshot::Sender<()>>, task: tokio::task::JoinHandle<RecorderResult<()>>,
...
impl RecordingHandle {
    fn native(handle: legacy_recorder::RecordingHandle) -> Self {
        Self { output_path: handle.output_path().to_owned(), status: handle.status().clone(),
               running: Some(Running::Native(handle)), completion_error: None }
    }
    ...
    pub async fn stop(&mut self) -> RecorderResult<()> {
        match self.running.as_mut() {
            ...
            Some(Running::Streamlink { stop, .. }) => { if let Some(stop) = stop.take() { let _ = stop.send(()); } }
            None => {}
        }
        self.finish().await?;
    ...
    pub async fn get_progress(&mut self) -> Option<RecordProgress> {
        ...
        if let Some(ref update) = result { self.status = update.status.clone(); }
        result
    }
    async fn finish(&mut self) -> RecorderResult<()> {
        if let Some(running) = self.running.take() {
            let result = match running {
                ...
                Running::Streamlink { task, progress, stop: _stop } => {
                    // Keep the stop sender alive while awaiting natural completion.
                    let result = task.await.map_err(|_| RecorderError::TaskCancelled).and_then(|r| r);
    ...
    pub async fn wait(mut self) -> RecorderResult<()> { self.finish().await }
    pub fn output_path(&self) -> &Path { &self.output_path }
    pub fn status(&self) -> &RecordStatus { &self.status }
}
...
mod tests {
    ...
    fn output_uses_plugin_not_hardcoded_douyin() {
        let room = LiveRoomInfo { room_id: "123".into(), anchor_name: "test".into(), title: "title".into(),
            status: crate::types::LiveStatus::Live, start_time: None, viewer_count: None, cover_url: None,
            extra: std::collections::HashMap::from([("streamlink_plugin".into(), json!("soop"))]) };
        let config = RecordConfig { output_path_template: "record/{platform}/{room_id}.ts".into(), format: "ts".into(), ..Default::default() };
        assert!(output_path(&room, &config).unwrap().ends_with("record/soop/123.ts"));
    }
}
