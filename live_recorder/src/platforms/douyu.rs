...
impl DouyuHandler {
    ...
    /// 从网页提取 JS 并生成签名参数
    async fn get_sign_params(&self, room_id: &str, did: &str) -> RecorderResult<HashMap<String, String>> {
        // 1. 请求房间页面
        ...
        let re_js = Regex::new(r"(var [a-zA-Z0-9_]+=\[0x[a-f0-9]+[\s\S]*?function ub98484234[\s\S]*?)function")
            .map_err(|e| RecorderError::InvalidResponseFormat(format!("Regex error: {}", e)))?;
        
        let js_code = re_js.captures(&html)
            .and_then(|c| c.get(1))
            .map(|m| m.as_str())
            .ok_or_else(|| RecorderError::InvalidResponseFormat("Cannot find JS code in page".to_string()))?;
        ...
        let runtime = Runtime::new()
            .map_err(|e| RecorderError::JavaScriptError(format!("Failed to create JS runtime: {}", e)))?;
        let context = Context::full(&runtime)
            .map_err(|e| RecorderError::JavaScriptError(format!("Failed to create JS context: {}", e)))?;
        
        // 执行第一段 JS
        let res: String = context.with(|ctx| {
            // 先执行函数定义
            ctx.eval::<(), _>(func_ub9.as_str()).catch(&ctx)
                .map_err(|e| RecorderError::JavaScriptError(format!("Failed to eval JS: {:?}", e)))?;
            
            // 调用 ub98484234
            let func: Function = ctx.globals().get("ub98484234").catch(&ctx)
                .map_err(|e| RecorderError::JavaScriptError(format!("Failed to get ub98484234: {:?}", e)))?;
            
            let result: String = func.call(()).catch(&ctx)
                .map_err(|e| RecorderError::JavaScriptError(format!("Failed to call ub98484234: {:?}", e)))?;
        ...
        let re_v = Regex::new(r"v=(\d+)")
            .map_err(|e| RecorderError::InvalidResponseFormat(format!("Regex error: {}", e)))?;
        let v = re_v.captures(&res)
            .and_then(|c| c.get(1))
            .map(|m| m.as_str())
            .ok_or_else(|| RecorderError::InvalidResponseFormat("Cannot find v parameter".to_string()))?;
        ...
        let params: String = context.with(|ctx| {
            // 执行 sign 函数定义
            ctx.eval::<(), _>(func_sign.as_str()).catch(&ctx)
                .map_err(|e| RecorderError::JavaScriptError(format!("Failed to eval sign JS: {:?}", e)))?;
            
            // 调用 sign(rid, did, t10)
            let func: Function = ctx.globals().get("sign").catch(&ctx)
                .map_err(|e| RecorderError::JavaScriptError(format!("Failed to get sign: {:?}", e)))?;
            
            let result: String = func.call((room_id, did, t10.as_str())).catch(&ctx)
                .map_err(|e| RecorderError::JavaScriptError(format!("Failed to call sign: {:?}", e)))?;
    ...
    async fn get_live_stream(&self, room_id: &str) -> RecorderResult<StreamInfo> {
        ...
        if error_code != 0 {
            ...
            return Err(RecorderError::StreamNotAvailable(format!(
                "Douyu API error {}: {}", error_code, msg
            )));
        }
...
impl PlatformHandler for DouyuHandler {
    ...
    async fn extract_room_id(&self, url: &str) -> RecorderResult<String> {
        ...
        let parsed = url::Url::parse(url)
            .map_err(|_| RecorderError::InvalidUrlFormat(url.to_string()))?;
        
        // 优先从查询参数获取
        if let Some(rid) = parsed.query_pairs().find(|(k, _)| k == "rid").map(|(_, v)| v.to_string()) {
            return Ok(rid);
        }
        ...
        Err(RecorderError::InvalidUrlFormat(format!(
            "Cannot extract room ID from URL: {}", url
        )))
    }
