...
use super::utils::{extract_percent, extract_speed, extract_total_size};
...
impl AppState {
    ...
    pub fn get_video_info_in_background(
        &self,
        url: String,
    ) -> std::thread::JoinHandle<Result<magekit_shared::VideoInfo>> {
    ...
    pub fn download_video_in_background(
        ...
        self.download_video_in_background_internal(
            task_id,
            url,
            output_dir,
            format_id,
    ...
    fn download_video_in_background_internal(
        ...
        std::thread::spawn(move || {
            ...
            if let Some(ref cookie_file) = cookie_file {
                tracing::info!("🍪 下载时使用 Cookie 文件: {:?}", cookie_file.path());
            ...
            let output_file: Arc<std::sync::Mutex<Option<std::path::PathBuf>>> =
                Arc::new(std::sync::Mutex::new(None));
            let all_lines: Arc<std::sync::Mutex<Vec<String>>> =
                Arc::new(std::sync::Mutex::new(Vec::new()));
            let last_total: Arc<std::sync::atomic::AtomicU64> =
            ...
            if !status.success() {
                let error_context: Vec<String> = all_lines_result
                    .iter()
                    .filter(|l| {
                        l.contains("ERROR")
                            || l.contains("error")
                ...
                let error_msg = if error_context.is_empty() {
                    let last_lines: Vec<String> = all_lines_result
                        .iter()
                        .rev()
                        .take(5)
                        .map(|s| s.clone())
            ...
            if final_output_file.is_none() {
                tracing::warn!(
                    "⚠️ 未从 yt-dlp 输出中检测到文件路径，尝试在输出目录查找最新文件..."
                );
