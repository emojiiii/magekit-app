...
use magekit_shared::{ToolType, create_command, get_tools_dir};
...
impl ToolStorage {
            ...
            ToolType::YtDlp => {
                ...
                version_output
                    ...
                    .and_then(|line| {
                        ...
                        line.split_whitespace().find(|s| s.starts_with("20"))
                    })
                    .map(|v| v.to_string())
            }
            ToolType::Ffmpeg => {
    ...
    pub fn delete_tool_sync(&self, tool_type: ToolType) -> ToolManagerResult<()> {
        let tool_path = self.get_tool_path(tool_type);
        if tool_path.exists() {
            std::fs::remove_file(&tool_path).map_err(|e| {
    ...
    pub async fn delete_tool(&self, tool_type: ToolType) -> ToolManagerResult<()> {
        let tool_path = self.get_tool_path(tool_type);
        if tool_path.exists() {
            tokio::fs::remove_file(&tool_path).await.map_err(|e| {
                ToolManagerError::file_operation_failed(
                    "delete tool",
                    format!("Failed to delete {}: {}", tool_path.display(), e),
                )
    ...
    fn default() -> Self {
        Self {
            tools_dir: get_tools_dir()
                .unwrap_or_else(|_| std::env::current_dir().unwrap().join("tools")),
        }
    }
}
